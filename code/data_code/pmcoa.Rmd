---
title: 'Registration review: PMCOA data preparation'
...
# Import data (from: https://osf.io/2f857/download)
pmcoa <- vroom("../../../transparency-indicators/transparency-indicators/4_automated-evaluation/data/tidy_data/indicators_all.csv")
```
...
Note that year refers to the year of publication - wherever this was not available, the year of electronic publication is used.

```{r}
pmcoa %<>% 
  select(
    pmcid_pmc:doi, year, journal, publisher, affiliation_country,
    is_art, field, score, jif, n_cite,
    is_register_pred:is_explicit_reg, protocol_1
  )
```
...
```{r save}
# Save
write_csv(pmcoa, "../../data/tidy_data/pmcoa.csv")
```
